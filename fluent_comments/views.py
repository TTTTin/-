...
def post_comment_ajax(request, using=None):
    ...
    if request.user.is_authenticated():
        ...
        #这里修改是为了能够登陆后正常使用，否则会需要BaseUser的email导致错误
    ...
    if comment.content_type == ContentType.objects.get(app_label="scratch_api", model="production"):
        obj = comment.content_type.get_object_for_this_type(pk=comment.object_pk)
        notify.send(sender=obj.author, recipient=obj.author, actor=comment.user, verb='评论了你的', target=obj,
                    description=comment.user.username + '刚刚评论了你的"' + obj.name + '"作品, 评论内容为"' + str(comment.comment) + '"快去看看吧!')
    elif comment.content_type == ContentType.objects.get(app_label="scratch_api", model="user"):
        obj = comment.content_type.get_object_for_this_type(pk=comment.object_pk)
        notify.send(sender=comment.user, recipient=obj, actor=comment.user, verb="在你的个人空间下留下了评论",
                    description=comment.user.username + '在你的个人空间下留下了评论，' + '评论内容为"' + str(comment.comment) + '"，快去看看吧!')
    elif comment.content_type == ContentType.objects.get(app_label='qa', model='question'):
        obj = comment.content_type.get_object_for_this_type(pk=comment.object_pk)
        notify.send(sender=comment.user, recipient=obj.user, actor=comment.user, verb="在你提的问题下留下了评论", target=obj,
                    description=comment.user.username+'在你的提的问题下留下了评论，评论内容为"'+str(comment.comment)+'"，快去看看吧')
    elif comment.content_type == ContentType.objects.get(app_label='qa', model='answer'):
        obj = comment.content_type.get_object_for_this_type(pk=comment.object_pk)
        notify.send(sender=comment.user, recipient=obj.user, actor=comment.user, verb="在你的回答下留下了评论", target=obj,
                    description=comment.user.username+'在你的回答下留下了评论，评论内容为"'+str(comment.comment)+'"，快去看看吧')
