    ...
    */
    ...
    $.fn.score.defaults = {
        ...
        click       : function(score, event){
            var id = this.id;
            id = Number(id.substring(11));
            var all_id = this.id;
            $.ajax({
                url: '/website/ajax/comment_production/' + productionids[id] + "/?score=" + score,
                success: function () {
                    var c = $("#" + all_id);
                    $(c).parent().find("p").first().html("班级互评得分：" + (totalScores[id] - haveScore[id] + score));
                    totalScores[id] = totalScores[id] - haveScore[id] + score;
                    haveScore[id] = score;
                    alert("评分成功!");
                }
            });
