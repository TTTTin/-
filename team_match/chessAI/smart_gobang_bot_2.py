...
class Node:
    ...
    def __init__(self, parent, depth, role, last_action, is_last_child):
        self.parent, self.children = parent, None
        self.role = role
        self.depth = depth
        self.score, self.last_action, self.next_action = MIN_INT, last_action, None
        self.is_last_child = is_last_child

    def next(self, max_depth):
        if self.is_leaf(max_depth) or self.is_last_child:
            return self.parent
        if self.is_last_child:
            return

    def is_leaf(self, max_depth):
    ...
    def expand(self, chessboard):
        if self.children is None:
            return
        self.children = {}
        for row, col in gen(chessboard, self.role):
            self.children[(row, col)] = Node(self, self.depth + 1, self.reverse_role(), (row, col))

    def up(self):
        cur_node = self
        if cur_node.parent is None:
            return
        if cur_node.depth % 2 == 1:
            if cur_node.parent.score < cur_node.score:
                cur_node.parent.score, cur_node.parent.next_action = cur_node.score, cur_node.last_action
        else:
            if cur_node.parent.score > cur_node.score:
                cur_node.parent.score, cur_node.parent.next_action = cur_node.score, cur_node.last_action

    def down(self, chessboard, max_depth):
        if self.depth == max_depth:
            return score(chessboard, self.reverse_role()) if self.depth % 2 == 1 else score(chessboard, self.role)





def score(chessboard, role):
    print(chessboard, role)
    return 0

def gen(chessboard, role):
    print(chessboard, role)
    return [[1, 2]]
